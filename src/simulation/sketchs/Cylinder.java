...
public class Cylinder extends SketchFragment implements GasObserver {
    ...
    public void fillCylinder(float initialVolume, int nParticle, float v) {
        ...
        float initialPistonHeight = PApplet.map(initialVolume, GasDataMap.MIN_PROCESS_VOLUME,
                GasDataMap.MAX_PROCESS_VOLUME, GasDataMap.MAX_PISTON_HEIGHT, GasDataMap.MIN_PISTON_HEIGHT);
    ...
    public void updateGasData(GasData gasData) {
        this.dvel = gasData.getGasData().get("deltaVel");
        this.pistonHeight = gasData.getGasData().get("fake_piston_height");
    }
    ...
    class Particle {
        ...
        public void checkParticleCollision(Particle p) {
            ...
            if (distanceVectMag < minDistance) {
                ...
                /*
                 * float distanceCorrection = (minDistance-distanceVectMag) / 2f; PVector d =
                 * distanceVect.copy(); PVector correctionVector =
                 * d.normalize().mult(distanceCorrection); p.position.add(correctionVector);
